...
export const LoggerPanel: React.FC<LoggerPanelProps> = ({ logs, className }) => {
  return (
    <div
      className={clsx(
        "flex flex-col bg-slate-900 text-slate-200 font-mono text-xs rounded-lg overflow-hidden border border-slate-700 h-[200px]",
        className
      )}
    >
      <div className="bg-slate-950 px-3 py-1 border-b border-slate-800 text-[10px] uppercase tracking-wider text-slate-500 font-bold flex justify-between items-center">
        ...
        <span
          className={clsx(
            ...
            logs.length > 0 ? "bg-green-500 animate-pulse" : "bg-slate-600"
          )}
        />
      </div>
