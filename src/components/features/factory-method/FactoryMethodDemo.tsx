...
export const FactoryMethodDemo = () => {
  ...
  const handleCreate = () => {
    ...
    setTimeout(() => {
      ...
      addLog(
        `Factory: Created new ${platform === "WEB" ? "Square" : "Rounded"} Button.`,
        "success"
      );
      setIsCreating(false);
    }, 1500);
        ...
        <SvgCanvas className="flex-1">
          ...
          <Node
            ...
            className={clsx(
              ...
              platform === "WEB"
                ? "fill-blue-100 stroke-blue-600 text-blue-900"
                : "fill-green-100 stroke-green-600 text-green-900"
            )}
          />
          {/* Pulse Effect for Factory */}
          {isCreating && (
            <motion.circle
              cx={nodes.factory.x}
              cy={nodes.factory.y}
              ...
              className={clsx(
                "fill-none stroke-2 pointer-events-none",
                platform === "WEB" ? "stroke-blue-400" : "stroke-green-400"
              )}
            />
          )}
              ...
              <AnimatePresence>
                {products.map((p) => (
                  <motion.div
                    ...
                    layout
                    className={clsx(
                      "w-20 h-10 shadow-md flex items-center justify-center text-[10px] text-white font-bold transition-colors border-2 border-white",
                      p.type === "WEB"
